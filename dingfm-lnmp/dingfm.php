...
$mail = 'test@gong.si';//若有恶意登录，会发邮件到这个邮箱，前提是mail()函数可用！
...
function login($er=false) {
    ...
    echo "<form action=\"".$meurl."?op=".$op."\" method=\"post\">\n"
        ."<input type=\"text\" name=\"user\" border=\"0\" class=\"login_text\" value=\"".@$user."\"  placeholder=\"请输入用户名\">\n"
        ."<input type=\"password\" name=\"pass\" border=\"0\" class=\"login_text\" value=\"".@$pass."\" placeholder=\"请输入密码\">\n"
...
function upload($upfile,$ndir,$unzip,$delzip) {
    ...
    while (count($upfile['name']) >= $i){
        ...
        echo "现在您可以 <a href=\"".$meurl."?op=home&folder=".$ndir."\">打开上传的文件夹</a> 或者 <a href=\"".$meurl."?op=home&folder=".$_SESSION['folder']."\">返回上次浏览的文件夹</a> 或者 <a href=\"".$meurl."?op=up\">上传其他文件</a>\n";
        echo '</div>';
        mainbottom();
    }else{
        printerror("您没有选择文件！");
    ...
    if (!$dename == "") {
        ...
        if(class_exists('ZipArchive')){
            ...
            if ($zip->open($folder.$dename) === TRUE) {
                ...
                echo "<a href=\"".$meurl."?op=home&folder=".$_SESSION['folder']."\">返回上次浏览的文件夹</a></div>\n";
                mainbottom();
            }else{
...
function edit($fename) {
    ...
    if (file_exists($folder.$fename)) {
        ...
        if($encode!=="UTF-8" && !empty($encode)){
            $contents = iconv("UTF-8", $encode."//IGNORE", $contents);
        }
...
function save($ncontent, $fename, $encode) {
    ...
    if($encode!=="UTF-8" && $encode!=="ASCII"){
    	$ydata = iconv($encode."//IGNORE", "UTF-8", $ydata);
    }

    if(file_put_contents($file, $ydata)) {
        echo "<div class='box'>文件 <a href=\"".$folder.$fename."\" target=\"_blank\">".$folder.$fename."</a> 保存成功！\n"
            ."请选择 <a href=\"".$meurl."?op=home&folder=".$_SESSION['folder']."\">返回上次浏览的文件夹</a> 或者 <a href=\"".$meurl."?op=edit&fename=".$fename."&folder=".$folder."\">继续编辑</a></div>\n";
        $fp = null;
    }else{
        echo "<span class='error'>文件保存出错！</span>\n"
        ." <a href=\"".$meurl."?op=home&folder=".$_SESSION['folder']."\">返回上次浏览的文件夹</a>\n";
    }
    mainbottom();
    }else{
    home();
...
function renam($rename, $nrename, $folder) {
    ...
    if (!$rename == "") {
        ...
        if(rename($loc1,$loc2)) {
            ...
            ." <a href=\"".$meurl."?op=home&folder=".$_SESSION['folder']."\">返回上次浏览的文件夹</a></div>\n";
            mainbottom();
        }else{
            printerror("重命名出错！");
        }
...
function mainbottom() {
    echo "</div><div style='text-align:center;font-size:13px;color:#999 !important;margin-top:5px;margin-bottom:45px'>"
        ."FileBox Version 1.7.0.1</div></body>\n"
        ."</html>\n";
    exit;
}
...
if(@$_POST['action']=='删除'){
    if(isset($_POST['select_item'])){
        ...
        echo "<a href=\"".$meurl."?op=home&folder=".$_SESSION['folder']."\">返回上次浏览的文件夹</a>\n";
        echo '</div>';
        mainbottom();
    }else{
        printerror("您没有选择文件");
